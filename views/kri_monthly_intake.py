...
MAKER_HISTORY_SELECT = """
    s.submission_id, s.reporting_period, s.actual_value_text, s.rag_status,
    s.remarks, s.review_notes, s.submitted_at, s.approved_by, s.approved_at,
    s.updated_at, s.workflow_status, c.entity, c.department, c.kri_title
"""
...
def _maker_submissions_df(
    *,
    workflow_status: str | None,
    lookback_months: int,
):
    clauses = [
        f"LOWER(s.submitted_by) = LOWER({user_lit})",
        f"s.submitted_at >= add_months(current_timestamp(), -{int(lookback_months)})",
    ]
    if user_dept:
        clauses.append(f"c.department = {sql_literal(user_dept)}")
    if workflow_status:
        clauses.append(f"s.workflow_status = {sql_literal(workflow_status)}")
    where = " AND ".join(clauses)
    return run_query(
        f"SELECT {MAKER_HISTORY_SELECT} "
        f"FROM {TBL_SUBMISSIONS} s "
        f"INNER JOIN {TBL_CATALOG} c ON s.kri_id = c.kri_id "
        f"WHERE {where} "
        f"ORDER BY s.reporting_period DESC, c.kri_title"
    )


def _render_maker_history_table(df, *, empty_message: str) -> None:
    if df.empty:
...
def render_submit_edit_form() -> None:
    entities = fetch_lookup("entity")
    col1, col2 = st.columns(2)
    with col1:
        entity = st.selectbox("Entity", entities, key="intake_entity")
    with col2:
        ...
        if user_dept:
            ...
            st.text_input("Department", value=department, disabled=True, key="intake_dept_locked")
        else:
            if not dept_list:
                st.info(
                ...
                st.stop()
            department = st.selectbox("Department", dept_list, key="intake_dept")
    ...
    kri_title = st.selectbox("KRI", kris_df["kri_title"].tolist(), key="intake_kri")
    ...
    if kri_unit and kri_unit != "Status / Narrative":
        st.caption(
            f"Expected format for **{kri_unit}**: "
            f"{UNIT_FORMAT_HINTS.get(kri_unit, 'a plain number, optionally with %')}."
        )
...
if role == "MAKER":
    st.write(
        "Submit or update KRI values for your department, or review your submission history "
        "and checker outcomes."
    )
    tab_submit, tab_all, tab_approved, tab_rejected = st.tabs(
        ["Submit / edit", "My submissions", "My approved", "My rejections"]
    )
    with tab_submit:
        render_submit_edit_form()
    with tab_all:
        lookback = st.selectbox(
            "Show submissions from the last",
            [3, 6, 12, 24],
            index=1,
            format_func=lambda m: f"{m} months",
            key="maker_all_lookback",
        )
        st.caption(f"All KRIs you submitted as **{user}** ({user_dept or 'department'}).")
        _render_maker_history_table(
            _maker_submissions_df(workflow_status=None, lookback_months=lookback),
            empty_message="No submissions from you in this window.",
        )
    with tab_approved:
        lookback_a = st.selectbox(
            "Show approved from the last",
            [3, 6, 12, 24],
            index=1,
            format_func=lambda m: f"{m} months",
            key="maker_approved_lookback",
        )
        st.caption("Submissions you submitted that a Checker **Approved**.")
        _render_maker_history_table(
            _maker_submissions_df(workflow_status="Approved", lookback_months=lookback_a),
            empty_message="No approved submissions from you in this window.",
        )
    with tab_rejected:
        lookback_r = st.selectbox(
            ...
            index=1,
            format_func=lambda m: f"{m} months",
            key="maker_rejected_lookback",
        )
        st.caption("Submissions returned to you for correction — edit them under **Submit / edit**.")
        _render_maker_history_table(
            _maker_submissions_df(workflow_status="Rejected", lookback_months=lookback_r),
            empty_message="No rejected submissions in this window.",
        )
else:
    st.write(
